    ...
    public class Matrix {
        ...
        public Matrix(double[][] data) : this(data.Length, data[0].Length) {
            for(int m = 0; m < Rows; m++)
                for(int n = 0; n < Columns; n++) Data[m][n] = data[m][n];
        }

        public Matrix(double[] data) : this(data.Length, 1) { //  Column Matrix
            for(int m = 0; m < Rows; m++) Data[m][0] = data[m];
        }
        ...
        private Matrix Minor(int row, int column) {
            ...
            for(int m1 = 0; m1 < Rows; m1++)
                if(m1 != row) {
                    for(int n1 = 0, n2 = 0; n1 < Columns; n1++)
                        if(n1 != column) {
        ...
        public static Matrix Identity(int rows, int columns) {
            ...
            for(int m = 0, n = 0; m < rows && n < columns; m++, n++) r.Data[m][n] = 1;
            return r;
        }
        ...
        public static Matrix operator *(Matrix mA, Matrix mB) { // Dot Product
            ...
            if(mA.Columns == mB.Rows) {
                ...
                throw new ArgumentException("The number of rows in m1 must match the number rows in m2");
            }
        ...
        public static Matrix operator /(Matrix m1, double s) => m1 * (1 / s);
        ...
        public string Print() {
            ...
            string result = $"{Rows}x{Columns}" + Environment.NewLine;
